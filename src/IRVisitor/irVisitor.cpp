...
void IRVisitor::visitLiteral(const Ast::Literal &literal) {
  ...
  if (type->isCharacterType())
    ret =
        ConstantInt::get(builder.getInt8Ty(), std::get<char>(literal.literal));
  else if (type->isIntegerType())
    ret =
        ConstantInt::get(builder.getInt32Ty(), std::get<int>(literal.literal));
  else if (type->isFloatType())
    ret =
        ConstantFP::get(builder.getFloatTy(), std::get<float>(literal.literal));
  else
...
void IRVisitor::visitFunctionCall(const Ast::FunctionCall &functionCall) {
  ...
  for (int i = 0; i < functionCall.arguments.size(); ++i) {
    ...
    if (i < function->arg_size())
      ret = cast(ret, function->args().begin()[i].getType());
...
void IRVisitor::visitIncludeStdioStatement(
  ...
  for (const auto &name : names) {
    const auto &printf =
        module
            .getOrInsertFunction(name, llvm::FunctionType::get(
