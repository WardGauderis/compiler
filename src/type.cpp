...
[[nodiscard]] std::string Type::string() const {
  return std::visit(
      overloaded{[&](std::monostate empty) { return std::string("void"); },
                 ...
                 [&](const ArrayType &arr) {
                   return arr.second->string() + '[' +
                          (arr.first ? std::to_string(arr.first) : "") + ']';
                 }},
      type);
}
...
bool Type::isConst() const { return isTypeConst; }

bool Type::isBaseType() const { return type.index() == 2; }

bool Type::isPointerType() const { return type.index() == 1; }
...
bool Type::isVoidType() const { return type.index() == 0; }

bool Type::isFunctionType() const { return type.index() == 3; }

bool Type::isArrayType() const { return type.index() == 4; }
...
bool operator!=(const Type &lhs, const Type &rhs) { return not(lhs == rhs); }
...
Type *Type::unary(PrefixOperation operation, Type *operand, size_t line,
                  size_t column, bool print) {
  if (operation == PrefixOperation::Deref) {
    if (operand->isPointerType()) {
      ...
      std::cout << SemanticError("cannot dereference non-pointer type " +
                                     operand->string(),
                                 line, column);
      return nullptr;
    }
  ...
  } else if ((operation == PrefixOperation::Plus or
              operation == PrefixOperation::Neg) and
             operand->isPointerType()) {
    if (print)
      std::cout << InvalidOperands(operation.string(), operand->string(), line,
                                   column);
    return nullptr;
  }
  return operand;
...
Type *Type::combine(BinaryOperation operation, Type *lhs, Type *rhs,
                    size_t line, size_t column, bool print) {
  if (operation.isLogicalOperator()) {
    return new Type(false, BaseType::Int); // TODO: make this a bool
  }

  if (operation.isComparisonOperator() and
      ((rhs->isPointerType() and lhs->isFloatType()) or
       (rhs->isFloatType() and lhs->isPointerType()))) {
    // empty statement, just go to throw
  } else if (lhs->isBaseType() and rhs->isBaseType()) {
    if (operation == BinaryOperation::Mod and
        (lhs->isFloatType() or rhs->isFloatType())) {
  ...
  } else if (lhs->isPointerLikeType() and rhs->isIntegralType() and
             operation.isAdditiveOperator()) {
    return lhs;
  } else if (lhs->isIntegralType() and rhs->isPointerLikeType() and
             operation == BinaryOperation::Add) {
    return rhs;
  }
  if (print)
    std::cout << InvalidOperands(operation.string(), lhs->string(),
                                 rhs->string(), line, column);
  return nullptr;
}

bool Type::convert(Type *from, Type *to, bool cast, size_t line, size_t column,
                   bool print) {
  ...
  if (to->isArrayType()) {
    if (print)
      std::cout << ConversionError(operation, from->string(), to->string(),
                                   line, column);
    return false;
  }

  // cannot convert from or to void
  if ((from->isVoidType() and not to->isVoidType()) or
      (not from->isVoidType() and to->isVoidType())) {
    if (print)
      std::cout << ConversionError(operation, from->string(), to->string(),
                                   line, column);
    return false;
  ...
  if (from->isPointerLikeType()) {
    if (to->isFloatType()) {
      ...
      if (print)
        std::cout << PointerConversionWarning(operation, "from", from->string(),
                                              to->string(), line, column);
  ...
  if (to->isPointerLikeType()) {
    if (from->isFloatType()) {
      ...
      if (print)
        std::cout << PointerConversionWarning(operation, "to", from->string(),
                                              to->string(), line, column);
    }
  }

  // casting to narrower type
  if (from->isBaseType() and to->isBaseType() and
      to->getBaseType() < from->getBaseType() and not cast) {
    std::cout << NarrowingConversion(operation, from->string(), to->string(),
                                     line, column);
  }
  // casting to narrower basetype
  if (not cast and from->isPointerType() and to->isPointerType() and
      (*from) != (*to)) {
    if (print)
      std::cout << PointerConversionWarning(operation, "to", from->string(),
                                            to->string(), line, column);
