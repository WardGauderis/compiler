...
Ast::Expr* visitLiteralOrVariable(antlr4::tree::ParseTree* context)
{
	...
	{
		return new Ast::Variable(nullptr, context->getText());
	}
...
Ast::Expr* visitPostfixExpr(antlr4::tree::ParseTree* context)
{
	...
	{
		const auto lhs = new Ast::Variable(nullptr, context->children[0]->getText());
...
Ast::Expr* visitprefixExpr(antlr4::tree::ParseTree* context)
{
	...
	visitor(2, [](auto* context)
	{
		const auto rhs = new Ast::Variable(nullptr, context->children[1]->getText());
...
Ast::Expr* visitAssignExpr(antlr4::tree::ParseTree* context)
{
	...
	visitor(3, [](auto* context)
	{
		...
		return new Ast::Assignment(new Ast::Variable(nullptr, identifier), rhs);
	});
	return visitor.result();
}
...
{
	Ast::Type* type = visitBasicType(context->children[0]);
	VisitorHelper<Ast::Type> visitor(context, "expression");
	visitor(1, [&](auto* context)
	{
		return type;
	});
	visitor(2, [&](auto* context)
	{
		type = visitPointerType(context->children[1], type);
		return type;
	});
	return visitor.result();
}
...
Ast::BasicType* visitBasicType(antlr4::tree::ParseTree* context)
{
	...
	return new Ast::BasicType(isConst, specifier->getText());
}

Ast::PointerType* visitPointerType(antlr4::tree::ParseTree* context, Ast::Type* baseType)
{
	if (context->children.size()==1)
		return new Ast::PointerType(false, baseType);
	else if (context->children.size()==3)
		return visitPointerType(context->children[2], new Ast::PointerType(true, baseType));
	else if (typeid(*context->children[1])==typeid(CParser::QualifierContext))
		return new Ast::PointerType(true, baseType);
	else
		return visitPointerType(context->children[1], new Ast::PointerType(false, baseType));
}
...
{
	Ast::Type* type = visitTypeName(context->children[0]);
	Ast::Variable* var = new Ast::Variable(type, context->children[1]->getText());
	Ast::Expr* expr = nullptr;

	VisitorHelper<Ast::Statement> visitor(context, "statement");
	visitor(2, [&](auto* context)
	{
		return new Ast::Declaration(var, expr);
	});
	visitor(4, [&](auto* context)
	{
		expr = visitExpr(context->children[3]->children[0]);
		return new Ast::Declaration(var, expr);
	});
	return visitor.result();
}
